...
public class IndexController {
    ...
    public ModelAndView goToPanel(HttpServletRequest request) {
        ...
        List<Integer> visistedList = userService.getVisited();
        session.setAttribute("visitedMaryland", visistedList.get(0));
        session.setAttribute("visitedWestVirginia", visistedList.get(2));
        session.setAttribute("visitedMissouri", visistedList.get(1));
        session.setAttribute("userList", userList);
        session.setAttribute("propertiesContent", userService.getProperties().trim());
        return mv;
    }
