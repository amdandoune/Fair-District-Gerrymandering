...
public class UsersController {
    ...
    public String registerUser(@RequestParam Map<String, String> allRequestParam) {
        if (userService.exists(allRequestParam.get("username"))) {
            ...
            try {
                ...
                user.setPassword(userService.encrypt(allRequestParam.get("password")));
    ...
    public ModelAndView loginUser(@RequestParam Map<String, String> allRequestParam, HttpServletRequest request) {
        currentUser = userService.login(allRequestParam.get("username"), userService.encrypt(allRequestParam.get("password")));
    ...
    public void changePassword(@RequestParam Map<String, String> allRequestParams, HttpServletRequest request) {
        HttpSession session = request.getSession(true);
        currentUser = (UsersEntity) session.getAttribute("currentUser");
        currentUser.setPassword(userService.encrypt(allRequestParams.get("password")));
        userService.update(currentUser);
    }
    ...
    @GetMapping(value = "/updateproperties")
    @ResponseBody
    public void updateProperties(@RequestParam Map<String, String> allRequestParams) {
        String updatedProperties = allRequestParams.get("propertiesContent");
        userService.writeProperties(updatedProperties);
    }
